## Сборка и запуск docker+django+sqlite3:

Должен быть запущен docker.
В папке проекта в терминале выполнить команды.

Сборка образа:
```bash
docker build -t stocks_products .
```
Запуск контейнера (порт 7070 - может быть любым):
```bash
docker run --name=stocks_products_server -d -p 7070:80 stocks_products
```
Проверка в терминале:
```bash
curl localhost:7070/api/v1/
```
Проверка в браузере: http://localhost:7070/api/v1/
#
#
